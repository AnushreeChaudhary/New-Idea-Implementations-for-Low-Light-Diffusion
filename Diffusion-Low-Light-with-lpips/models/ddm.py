...
class DenoisingDiffusion(object):
    ...
    def estimation_loss(self, x, output):
        ...
        lpips_loss = self.lpips_loss(pred_x, gt_img).mean()
        
        # Set a weight for LPIPS loss (you can configure this in your config file)
        lambda_lpips = self.config.lambda_lpips if hasattr(self.config, 'lambda_lpips') else 0.5
        
        # Combine photo loss components
        photo_loss = content_loss + ssim_loss + lambda_lpips * lpips_loss
