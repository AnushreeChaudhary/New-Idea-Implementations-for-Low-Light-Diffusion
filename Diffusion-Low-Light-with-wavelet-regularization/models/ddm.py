...
def high_frequency_loss(V_high, H_high, D_high):
    # L1 regularization for sharpness preservation in high-frequency components
    return torch.sum(torch.abs(V_high)) + torch.sum(torch.abs(H_high)) + torch.sum(torch.abs(D_high))
...
class DenoisingDiffusion(object):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                if self.step % 10 == 0:
                    print("step:{}, lr:{:.6f}, noise_loss:{:.4f}, photo_loss:{:.4f}, "
                          "frequency_loss:{:.4f}, high_freq_loss:{:.4f}".format(
                              self.step, self.scheduler.get_last_lr()[0],
                              noise_loss.item(), photo_loss.item(),
                              frequency_loss.item(), high_frequency_loss.item()))
    ...
    def estimation_loss(self, x, output, beta=0.000001):
        ...
        # =============wavelet coefficient regularization==================
       
        high_freq_loss = high_frequency_loss(input_high0, input_high1, gt_high1)  

        return noise_loss, photo_loss, frequency_loss, (beta * high_freq_loss)
